...
import { KorapayConfig } from '@/interface/korapaycheckoutInterface';
...
export const useKorapayCheckout = ({
  ...
  const injectedJavaScript = `
    function loadKorapayScript() {
      ...
      script.onerror = function() {
        window.ReactNativeWebView.postMessage(JSON.stringify({
          type: 'ERROR',
          data: { message: 'Failed to load Korapay script' }
        }));
      };
    ...
    const initializeKorapay = () => {
      try {
        if (!window.Korapay) {
          throw new Error('Korapay object not found');
        }
        ...
        window.ReactNativeWebView.postMessage('KORAPAY_INITIALIZED');
      } catch (error) {
        window.ReactNativeWebView.postMessage(JSON.stringify({
          type: 'ERROR',
